...
namespace AssemblyCSharp.Functions
{
	public static class KOLTracker
	{
		...
		// Giữ hook tương thích với GClass73 nhưng không ghi nhớ/replay packet NPC.
		// KOL chỉ còn passive để không làm lệch state NPC khi chuyển map.
		public static void ObserveMenuRequest(int npcId, int menuId, int optionId)
		{
		}

		// Background polling đã tắt hoàn toàn.
		...
		public static bool ObserveNpcDialog(int npcId, string chat, string[] menu)
		{
			...
			{
				// KOL là tính năng phụ: lỗi parser không được phép chặn flow NPC.
			}
			return false;
		}
		...
		public static bool ObserveNpcSay(int npcId, string chat)
		{
			...
			{
				// Fail-safe: không để tracker ảnh hưởng dialog game.
			}
			return false;
		}
